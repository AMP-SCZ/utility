...
4. If the participant is a CHR, the participant has to fulfill the criteria for the SIPS and/or CAARMS group.
5. If the participant is a HC, the participant does not fulfill criteria ofany of the SIPS and CAARMS group.
6. Additionally, the screening PSYCHS forms have no QC issues

5 and 6 can be simplified to: Inconsistecy between the SIPS/CAARMS group and the participant's cohort
"""
import json
import logging
...
def get_ndar_subject01_df() -> pd.DataFrame:
    ...
    """
    ndar_csvs = list(Path("/data/predict1/to_nda/nda-submissions").glob("ndar_subject01*.csv"))
...
def fix_ndar_csv(
    ...
    with open(ndar_csv, "r") as f:
        first_line = f.readline()

    with open(ndar_like_csv, "r") as f:
        lines = f.readlines()

    with open(ndar_like_csv, "w") as f:
        f.write(first_line)
        f.writelines(lines)
...
def select_subjects() -> Tuple[List[str], List[Dict[str, Any]], pd.DataFrame]:
    ...
    for idx, row in ndar_df.iterrows():
        ...
        if subject_json is None:
            ...
            skipped_subjects.append({"subject_id": subject_id, "reason": "Cannot find subject_json"})
            skip_subject = True
            continue
        ...
        if tracker_row.empty:
            skipped_subjects.append({"subject_id": subject_id, "reason": "tracker_row empty"})
            skip_subject = True
            # print(f"Skipping {subject_id} because tracker_row is empty")
            continue
        # inclusionexclusion_criteria_review_screening check
        inclusionexclusion_criteria_review_screening = tracker_row["inclusionexclusion_criteria_review_screening"].values[0]

        if not pd.isna(inclusionexclusion_criteria_review_screening):
            skipped_subjects.append({"subject_id": subject_id, "reason": f"inclusionexclusion_criteria_review_screening is {inclusionexclusion_criteria_review_screening}"})
            skip_subject = True
            # print(f"Skipping {subject_id} because inclusionexclusion_criteria_review_screening is {inclusionexclusion_criteria_review_screening}")
            continue
        ...
        if not pd.isna(psychs_p1p8_screening):
            skipped_subjects.append({"subject_id": subject_id, "reason": f"psychs_p1p8_screening is {psychs_p1p8_screening}"})
            skip_subject = True
            # print(f"Skipping {subject_id} because psychs_p1p8_screening is {psychs_p1p8_screening}")
            continue
        ...
        if not pd.isna(psychs_p9ac32_screening):
            skipped_subjects.append({"subject_id": subject_id, "reason": f"psychs_p9ac32_screening is {psychs_p9ac32_screening}"})
            skip_subject = True
            # print(f"Skipping {subject_id} because psychs_p9ac32_screening is {psychs_p9ac32_screening}")
            continue

        # subject_is_included
        chrcrit_included = get_variable_value(json_file=subject_json, redcap_event_name="screening", redcap_variable_name="chrcrit_included")
        subject_is_included = chrcrit_included == "1"

        if not subject_is_included:
            skipped_subjects.append({"subject_id": subject_id, "reason": f"chrcrit_included is {chrcrit_included}"})
            skip_subject = True
        ...
        if not skip_subject:
            ...
            new_ndar_df = pd.concat([new_ndar_df, pd.DataFrame([row])], ignore_index=True)
