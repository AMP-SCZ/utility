...
# cleaning up Kevin's work to make DPdash compatible files
...
# df_pivot = pd.pivot_table(df, index=['subject', 'site'], columns=['level0', 'level1', 'mtime'], fill_value=False).astype(int)
df_pivot = pd.pivot_table(df, index=['subject', 'site'], columns=['level0', 'level1'], fill_value=False).astype(int)


for (subject, site), row in df_pivot.iterrows():
    df_tmp = row.reset_index()
    # df_tmp.columns = ['datatype', 'level0', 'level1', 'mtime', 'count']
    # df_tmp_pivot = pd.pivot_table(df_tmp, columns=['datatype', 'level0', 'level1', 'mtime']).reset_index()
