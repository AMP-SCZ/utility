...
def create_fake_upenn_json_prescient(fake_json: Path, item_nums: int = 2):
    '''Create fake json'''
    ...
    if item_nums == 2:
        fake_dict_list = [
                {"session_battery": "SPLLT-A"},
                {"session_battery": "PRESCIENT_NOSPLLT_C1"}
                ]
    elif item_nums == 1:
        fake_dict_list = [
                {"session_battery": "PRESCIENT_NOSPLLT_C1"}
...
def test_check_upenn_cnb_pronet():
    subject = 'YA00000'
...
def test_check_upenn_cnb_prescient():
    subject = 'BM00000'
    phoenix_fake = test_dir / 'test_PHOENIX'
    site = subject[:2]
    ...
    create_fake_upenn_json_prescient(fake_json, item_nums=1)
    assert check_upenn_cnb(subject_raw_path) == 0

    create_fake_upenn_json_prescient(fake_json, item_nums=2)
    assert check_upenn_cnb(subject_raw_path) == 1

    create_fake_upenn_json_prescient(fake_json, item_nums=3)
    assert check_upenn_cnb(subject_raw_path) == 0
